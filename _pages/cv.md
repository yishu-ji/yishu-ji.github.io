---
layout: archive
title: "CV"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

[Download the pdf version here.](https://yishu-ji.github.io/files/YishuJI_s_CV.pdf)

📚 Education
======
* Ph.D. in Human-Centered Computing, Georgia Tech, 2029 (expected)
* B.Eng. in Industrial Engineering, Tsinghua University, 2024
* Minor in Psychology, Tsinghua University, 2024
* Exchange program in Information Science, Cornell University, Spring 2023

🧐 Research experience
======
* Summer 2025: Research Assistant @ **Columbia SEA Lab**
  * Advised by Prof. Orson "Xuhai" Xu
    * Leverage GenAI tools to make depression inference from wearable data.
    * Conduct data analysis for a longitudinal diary study exploring use of LLM in healthcare seeking journey.

* 2024 – 2025: Research Assistant @ **Gatech VisualizaXiong Lab**
  * Advised by Prof.Cindy Xiong Bearfield
    * Conduct empirical studies to examine human perception towards approximated visualization.
    * Explore the impact of dynamic visual cues in pursuit detection.

* Summer 2023: Research Assistant @ **Cornell University CIS**
  * Advised by Prof. Jeff Rzeszotarski and Dr. Nathan Yan
    * Built up novel tools with LLMs that could perform automatic data analysis and visualization.
    * Designed and implemented user studies to examine the assistive performance of LLMs on data exploration.
    * Constructed behavioral models for data exploration processes, and developed design prototypes.

* 2022 - 2023: Research Assistant @ **PI Lab at Tsinghua University**
  * Advised by Prof. Yukang Yan
    * Designed a user study and developed a Unity 3D-based study platform to conduct the experiment.
    * Developed a computational model of user’s perception of inconsistency in User-Avatar movement in VR.
   
* Summer 2022: Research Assistant @ **Tsinghua IE HCI Lab**
  * Advised by Prof. Pei-Luen Patrick Rau
    * Performed an experiment design and implemented the study platform with robot NAO. And conducted a user study with EEG to collect sensing and survey data in credit assignment in HRI scenarios.

📑 Publications
======
* Zhipeng Li, **Yishu Ji**, Ruijia Chen, Tianqi Liu, Yuntao Wang, Yuanchun Shi, Yukang Yan. **"Modelling Effects of Visual Attention on Noticeability of Body-Avatar Offsets in Virtual Reality"**. In: *Proceedings of the CHI Conference on Human Factors
in Computing Systems.* CHI’25
* Fengyuan Liu, **Yishu Ji**, Xin Lei, Pei-Luen Patrick Rau. **”Responses to Human and Robot Errors in Human–robot Collaboration: An fNIRS Study”**. In: *Proceedings of the International Conference on Human-Computer Interaction.* HCII’24

🎤 Posters and Presentations
======
* **Yishu Ji**, Maria Kon, Andrew Lovett, Mark Livingston, Sangeet Khemlani, Yalong Yang, Cindy Xiong Bearfield. **“Visualizing Motion Traces Enhances Pursuit Detection in Dynamic Scenes”**. Poster presented at: The Annual Meeting of the Cognitive
Science Society. CogSci’25

🏆 Awards
======
* Outstanding Undergraduate of Tsinghua University (Magna Cum Laude, Top 10%), 2024
* Tsinghua University ”December 9th” Award (Top 2%), 2021
* Tsinghua Comprehensive Excellence Honor (Top 15%), 2023, 2022, 2021, 2020

⚙️ Skills
======
* Programming languages (Python, C etc.)
* Data analysis tools (R, SPSS etc.)
* Data Visualization (Tableau)
* Game engines (Unity)
* Database system (SQL)
* User-centered design (UCD)
* Experiment design
* Qualitative & quantitative research method
